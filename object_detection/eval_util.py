...
def create_report(model_name, metrics, filename):
  """Write metrics to report.

  Args:
    metrics: A dictionary containing metric names and values.
    filename: Full path to file where report is written
  """
  logging.info('Writing metrics to report file.')
  mean_ap = metrics['PASCAL/Precision/mAP@0.5IOU']
  precision = {}
  for key in metrics.keys():
    category = key.split('/')[-1]
    if category != 'mAP@0.5IOU':
      precision[category] = metrics[key]

  od = collections.OrderedDict(sorted(precision.items()))
  ...
  with open(filename,'a') as f:
    ...
    key_count=1
    f.write(model_name.split('-')[1]+',')
    for key in od.keys():
      if key_count < len(od):
        f.write(str(od[key])+',')
      else:
        f.write(str(od[key])+','+str(mean_ap)+'\n')
      key_count = key_count + 1
    ...
    try:
      ...
      for batch in range(int(num_batches)):
        ...
        if (batch + 1) % 100 == 0:
          ...
          #print('Running eval ops batch %d/%d - time:%f', batch + 1, num_batches, time.time()-start)
...
def _checkpoint_run_once(checkpoint_file,
                         tensor_dict,
                         evaluators=None,
                         batch_processor=None,
                         checkpoint_dirs=None,
...
# TODO: Add tests.
def checkpoint_run_loop(report_filename, tensor_dict,
                            summary_dir,
                            evaluators,
                            batch_processor=None,
                            checkpoint_dirs=None,
                            ...
                            save_graph=False,
                            save_graph_dir=''):
  """Evaluates desired tensors for all checkpoints using checkpoint_dirs.
  ...
  iterlist = []
  for name in sorted(glob.glob(checkpoint_dirs[0]+'/*')):
    iterval = name.split('/')[-1].split('.')[0].split('-')[-1]
    if iterval == 'model':
      splits=name.split('/')[-1].split('.')
      checkpoint_file = splits[0]+'.'+splits[1]
      if checkpoint_file not in iterlist:
        iterlist.append(checkpoint_file)
  #print(iterlist)

  #pdb.set_trace()
  if len(iterlist) == 0:
    raise ValueError('No model files in the checkpoint directory. ')

  for checkpoint_file in iterlist:
    start = time.time()
    logging.info('Starting evaluation at ' + time.strftime(
        '%Y-%m-%d-%H:%M:%S', time.gmtime()))
    model_path = checkpoint_dirs[0]+'/'+checkpoint_file
    #pdb.set_trace()
    global_step, metrics = _checkpoint_run_once(model_path,tensor_dict, evaluators,
                                                batch_processor,
                                                checkpoint_dirs,
                                                variables_to_restore,
                                                num_batches,
    ...
    #pdb.set_trace()
    write_metrics(metrics, global_step, summary_dir)
    create_report(checkpoint_file, metrics, report_filename)
    #number_of_evaluations += 1
    ...
    #if time_to_next_eval > 0:
    #  time.sleep(time_to_next_eval)
    print('Time to complete eval: ',time.time()-start)
