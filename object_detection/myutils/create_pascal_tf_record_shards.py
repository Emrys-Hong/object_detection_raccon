...
def main(_):
  ...
  for shard_id in range(FLAGS.NUM_SHARDS):
    output_filenames.append(_get_dataset_filename(
      FLAGS.output_path, FLAGS.set, shard_id, FLAGS.NUM_SHARDS))

  label_map_dict = label_map_util.get_label_map_dict(FLAGS.label_map_path)

  logging.info('Reading from PASCAL %s dataset.', FLAGS.year)
  examples_path = os.path.join(data_dir, FLAGS.year, 'ImageSets', 'Main', FLAGS.set + '.txt')
  annotations_dir = os.path.join(data_dir, FLAGS.year, FLAGS.annotations_dir)
  examples_list = dataset_util.read_examples_list(examples_path)
  #pdb.set_trace()
  num_per_shard = int(math.ceil(len(examples_list) / float(FLAGS.NUM_SHARDS)))
